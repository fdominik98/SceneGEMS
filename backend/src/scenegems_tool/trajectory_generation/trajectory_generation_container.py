...
from utils.file_system_utils import TRAJECTORY_GENERATION_GEN_FOLDER, merge_compose_volumes

_COMPOSE_PROJECT = "scenegems-trajectory-generation-subsystem"
_SERVICE_NAME = "trajectory_generation"
_DEFAULT_IMAGE = "scenegems-trajectory-generation-subsystem:latest"
# One worker only for now (see spec): the service process plus a single RRT worker.
_CONTAINER_CPUS = 2
...
class TrajectoryGenerationContainer:
    """Owns Docker Compose lifecycle for the trajectory generation subsystem container."""
    ...
    def __init__(self, mqtt_connection: MQttConnectionInfo, reference_geofence: Geofence):
        ...
        self.compose_filename = f"{TRAJECTORY_GENERATION_GEN_FOLDER}/docker-compose.yml"
        ...
        self._lifecycle_thread = threading.Thread(
            ...
            name="trajectory_generation_compose",
            daemon=True,
        )
        self._lifecycle_thread.start()

    def _build_compose(self) -> dict:
        image = os.environ.get("TRAJECTORY_GENERATION_IMAGE", _DEFAULT_IMAGE)
        environment = {
        ...
        service = {
            ...
            "networks": ["trajectory_generation_net"],
        }
            ...
            attach_broker_network_to_compose(
                {
                    ...
                    "networks": {
                        "trajectory_generation_net": {
                            "name": "trajectory_generation_net",
                            "driver": "bridge",
                        }
                    },
                }
    ...
    def _run_container_lifecycle_startup(self, docker_compose: dict, compose_filename: str) -> None:
        try:
            ...
            if self._shutdown.is_set():
                return
            if os.path.exists(TRAJECTORY_GENERATION_GEN_FOLDER):
                shutil.rmtree(TRAJECTORY_GENERATION_GEN_FOLDER)
            os.makedirs(TRAJECTORY_GENERATION_GEN_FOLDER, exist_ok=True)
            if self._shutdown.is_set():
                return
            ...
            _logger.exception("TrajectoryGenerationContainer container lifecycle startup failed")
